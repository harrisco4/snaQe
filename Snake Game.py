...
screen = pygame.display.set_mode((1324, 640))
...
def wave(num):
    for x in range(0, 1324+10, int(jmp.val)):
        ...
        if radius_1 > radius_2:  # draw the smaller circle before the larger one
            pygame.draw.circle(screen, xcolor(int(x + 1324//2) + num * flow), (x, y_2), radius_2, 0)
...
xcolor = Gradient(COLORS, 1324).gradient
...
class App:
    ...
    def __init__(self):
        self.speed = Slider("Speed", 2.5, 10, 1, 1115)
        self._running = True
    ...
    def on_loop(self):
        ...
        for i in range(0,self.player.length):
            if self.game.isCollision(self.apple.x,self.apple.y,self.player.x[i], self.player.y[i],44):
                ...
                while True:
                    self.apple.x = randint(2,9) * 64
                    self.apple.y = randint(2,9) * 64
        ...
        if self.player.x[0] < 0 or self.player.x[0] > 1024:
            self.reset()
        if self.player.y[0] < 0 or self.player.y[0] > 640:
            self.reset()
